...
function App() {
  ...
  const galleryImages = [
    { id: 1, src: flooring, alt: 'Decking Repairs' },
    { id: 2, src: tiling, alt: 'Bathroom Tiling' },
    { id: 3, src: door, alt: 'Wood Repair' },
    { id: 4, src: floor, alt: 'Floor Installation' },
    { id: 5, src: book_shelve, alt: 'Bespoke Shelving' },
    { id: 6, src: shelf, alt: 'Bespoke Shelving'},
  ];

  return (
    <div className="min-h-screen bg-gray-50">
      {/* Fixed Navigation */}
      <nav className="bg-[#2E8B57] text-white shadow-lg fixed w-full top-0 z-50">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex space-x-8">
              <button onClick={() => scrollToSection('services')} className="hover:text-slate-200 transition-colors">Services</button>
              <button onClick={() => scrollToSection('about')} className="hover:text-slate-200 transition-colors">About us</button>
              <button onClick={() => scrollToSection('gallery')} className="hover:text-slate-200 transition-colors">Our work</button>
              <button onClick={() => scrollToSection('contact')} className="hover:text-slate-200 transition-colors">Contact</button>
            </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-16 left-0 right-0 bg-[#2E8B57] shadow-lg z-50">
            <div className="px-4 py-2 space-y-2">
              <button onClick={() => scrollToSection('services')} className="block w-full text-left py-2 px-4 rounded hover:bg-[#2E8B57]">Services</button>
              <button onClick={() => scrollToSection('about')} className="block w-full text-left py-2 px-4 rounded hover:bg-[#2E8B57]">About us</button>
              <button onClick={() => scrollToSection('gallery')} className="block w-full text-left py-2 px-4 rounded hover:bg-[#2E8B57]">Our work</button>
              <button onClick={() => scrollToSection('contact')} className="block w-full text-left py-2 px-4 rounded hover:bg-[#2E8B57]">Contact</button>
            </div>
          </div>
        )}
      </nav>
      ...
      <div className="pt-16">
        ...
        <section id="home" className="relative min-h-screen flex items-center">
          <div className="absolute inset-0 bg-gradient-to-br from-[#2E8B57] via-[#2E8B57] to-[#2E8B57] opacity-90"></div>
            ...
            <div className="text-center text-white space-y-8">
              <div className="flex justify-center mb-8">
                <div className="rounded-2xl">
                  <img src={logo} alt="SP Carpentry & Home Maintenance Logo" className="h-72 w-auto mx-auto" />
                </div>
              </div>
              ...
              <div className="flex flex-wrap justify-center gap-4">
                <button 
                  ...
                  className="bg-white text-[#2E8B57] px-8 py-4 rounded-lg font-semibold hover:bg-slate-100 transition-all duration-300 transform hover:scale-105 shadow-lg"
                >
                  Get Your Free Quote
                </button>
                <button 
                  onClick={() => scrollToSection('services')}
                  className="bg-transparent border-2 border-white text-white px-8 py-4 rounded-lg font-semibold hover:bg-white hover:text-[#2E8B57] transition-all duration-300"
                >
                  View Services
                </button>
              </div>
              ...
              <div className="flex flex-wrap justify-center gap-4 mb-8">
                <button
                  ...
                  className={`px-6 py-3 rounded-lg font-semibold transition-all duration-300 ${
                    activeServiceTab === 'maintenance' ? 'bg-[#2E8B57] text-white shadow-lg' : 'bg-white text-gray-700 hover:bg-slate-100'
                  }`}
                >
                  <Home className="w-5 h-5 inline mr-2" />Home Maintenance
                </button>
                <button
                  onClick={() => setActiveServiceTab('carpentry')}
                  className={`px-6 py-3 rounded-lg font-semibold transition-all duration-300 ${
                    activeServiceTab === 'carpentry' ? 'bg-[#2E8B57] text-white shadow-lg' : 'bg-white text-gray-700 hover:bg-slate-100'
                  }`}
                >
                  <PenLine className="w-5 h-5 inline mr-2" />Carpentry
                </button>
                <button
                  onClick={() => setActiveServiceTab('handyman')}
                  className={`px-6 py-3 rounded-lg font-semibold transition-all duration-300 ${
                    activeServiceTab === 'handyman' ? 'bg-[#2E8B57] text-white shadow-lg' : 'bg-white text-gray-700 hover:bg-slate-100'
                  }`}
                >
                  <Hammer className="w-5 h-5 inline mr-2" />Handyman
                </button>
              </div>
              ...
              <div className="bg-white p-8 rounded-xl shadow-lg">
                {activeServiceTab === 'carpentry' && (
                  <div className="space-y-6">
                    <div className="border-l-4 border-[#2E8B57] pl-6 py-4 hover:bg-slate-50 transition-colors rounded">
                ...
                <div className="mt-8 pt-6 border-t text-center">
                  ...
                  <button 
                    onClick={() => scrollToSection('contact')}
                    className="bg-[#2E8B57] text-white py-3 px-8 rounded-lg hover:bg-[#2E8B57] transition-all duration-300 transform hover:scale-105 shadow-lg inline-flex items-center"
                  >
                ...
                <div className="relative">
                  <div className="aspect-square rounded-2xl shadow-xl overflow-hidden">
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="aspect-square rounded-2xl shadow-xl overflow-hidden">
                        <img src={steve} alt="Steve Pond" className="w-full h-full object-contain p-8" />
                      </div>
                    </div>
                  </div>
                  <div className="absolute -bottom-6 -right-6 w-32 h-32 bg-[#2E8B57] rounded-2xl -z-10"></div>
                </div>

                <div className="space-y-6">
                  <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-[#2E8B57]">
                    <h3 className="text-xl font-semibold mb-3 text-gray-800">Expert Craftsmanship</h3>
                  ...
                  <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-[#2E8B57]">
                    <h3 className="text-xl font-semibold mb-3 text-gray-800">No Job Too Small</h3>
                    <p className="text-gray-700 leading-relaxed">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {galleryImages.map((item) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-[#2E8B57] bg-opacity-0 group-hover:bg-opacity-70 transition-all duration-300 flex items-center justify-center">
                      <p className="text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300 font-semibold px-4">{item.alt}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                  <form action="https://api.web3forms.com/submit" method="POST" className="space-y-4">
                    ...
                    <div>
                      ...
                      <input type="text" name="name" required minLength={2} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#2E8B57] focus:ring-[#2E8B57]" />
                    </div>
                    <div>
                      <label htmlFor="email" className="block text-sm font-medium text-gray-700">Email</label>
                      <input type="email" name="email" required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#2E8B57] focus:ring-[#2E8B57]" />
                    </div>
                    <div>
                      <label htmlFor="phone" className="block text-sm font-medium text-gray-700">Phone</label>
                      <input type="tel" name="phone" required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#2E8B57] focus:ring-[#2E8B57]" />
                    </div>
                    <div>
                      <label htmlFor="message" className="block text-sm font-medium text-gray-700">Message</label>
                      <textarea name="message" rows={4} required minLength={10} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#2E8B57] focus:ring-[#2E8B57]"></textarea>
                    </div>
                    <input type="checkbox" name="botcheck" className="hidden" style={{display: 'none'}} />
                    <button type="submit" className="w-full bg-[#2E8B57] text-white py-2 px-4 rounded-md hover:bg-[#2E8B57] transition-colors">
                      Send Message
                    </button>
                  </form>
                </div>
        ...
        <footer className="bg-gradient-to-r from-[#2E8B57] to-[#2E8B57] text-white py-8"> 
          <div className="container mx-auto px-4 text-center">
            <div className="flex items-center justify-center space-x-2 mb-4">
              <span className="font-bold text-lg">SP Carpentry & Home Maintenance</span>
            </div>
            <p className="text-white/80">35+ Years of Quality Craftsmanship</p>
            <p className="text-white/80 mt-2">Chelmsford, Essex</p>
          </div>
        </footer>
      </div>
    </div>
